    ...
    stages {
        ...
        stage('Docker Login') {
            steps {
                script {
                    sh """
                        echo "${ARTIFACTORY_PASSWORD}" | docker login -u"${ARTIFACTORY_USERNAME}" "${ARTIFACTORY_URL}"
                    """
                }
            }
        }
        ...
        stage('Docker Build and Publish') {
            steps {
                script {
                    ...
                    docker.withRegistry("https://${ARTIFACTORY_URL}", credentials('b3568e44-b80f-4700-8194-fd0547ee6230')) {
